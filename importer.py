                    ...
                    if start_found:
                        if account == CC_LUX or account == CE_LUX:
                            if row[1] != "DECOMPTE VISA":
                                ...
                                transacts.append(Transaction(-1,datetime.strptime(row[0],"%d/%m/%Y"),row[1],recipient,reference,float(row[2].replace(',', '.')),row[3],'',account,[]))
                        elif account == GK_DE and not (row[3] == "" and row[8] == ""):
                            amount_str = ('-') * (row[12] == 'S') + row[11].replace(',', '.')
                            amount = float(amount_str)
                            transacts.append(Transaction(-1,datetime.strptime(row[0], '%d.%m.%Y'), row[2], row[3],
                            ...
                            if row[3] != "Bank Deposit to PP Account" and row[3] != "Reversal of General Account Hold" and row[3] != "Account Hold for Open Authorization" and row[4] == "EUR":
                                ...
                                transacts.append(Transaction(-1,date, row[3], row[11], '', amount, row[4], '', account,[]))
                        elif account == VISA:
                            ...
                            transacts.append(Transaction(-1,date, 'Credit Card Transaction', recipient, reference, amount, row[4], '', account,[]))
        if account != PP:
            transacts.reverse()
        return transacts
